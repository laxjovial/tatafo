...
async def search_educational_resources(
    ...
    provider: str = "coursera", # Example provider
    user_api_keys: List[str] = []
) -> str:
    """
    Searches for educational resources (e.g., courses, tutorials, articles) based on a query.
    Optionally filters by resource type (e.g., "course", "video", "article").
    This tool integrates with educational content APIs.
    Requires 'education_tool_access' capability.

    Args:
        query (str): The educational search query (e.g., "machine learning course", "history of ancient Rome").
        resource_type (str, optional): The type of resource to search for. Defaults to None.
        user_context (UserProfile, optional): The user's profile for RBAC checks. Defaults to None.
        provider (str, optional): The API provider to use. Defaults to "coursera" (example).
    ...
    logger.info(f"Tool: search_educational_resources called for query: '{query}', resource_type: '{resource_type}', provider: '{provider}', user: '{user_context.user_id}'")
    ...
    if not get_user_tier_capability(user_context.user_id, 'education_tool_access', False, user_tier=user_context.tier, user_roles=user_context.roles):
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "permission_denied",
                                    {"tool_name": "search_educational_resources", "query": query, "provider": provider}, success=False)
    ...
    api_data = await make_api_request(
        ...
        function_name="search_educational_resources",
        params=params,
    ...
    if api_data:
        ...
        if results:
            result_str = f"Educational Resources for '{query}':\n" + "\n".join(results)
            analytics_tracker.log_event(user_context.user_id, "tool_usage", "success",
                                        {"tool_name": "search_educational_resources", "query": query, "provider": provider, "num_results": len(api_data.get("results", []))}, success=True)
            return result_str
        else:
            analytics_tracker.log_event(user_context.user_id, "tool_usage", "no_data",
                                        {"tool_name": "search_educational_resources", "query": query, "provider": provider, "message": "No educational resources found."}, success=False)
            return f"No educational resources found for query: '{query}'."
    else:
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "api_error",
                                    {"tool_name": "search_educational_resources", "query": query, "provider": provider, "message": "API call failed."}, success=False)
        return "Error: Could not search educational resources at this time. Please try again later."


@tool
async def education_search_web(
    query: str,
    user_context: Optional[UserProfile] = None,
    max_chars: int = 2000
) -> str:
    ...
    Args:
        query (str): The education-related search query (e.g., "best online learning platforms", "quantum physics basics explained").
    ...
    if not get_user_tier_capability(user_context.user_id, 'web_search_enabled', False, user_tier=user_context.tier, user_roles=user_context.roles):
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "permission_denied",
                                    {"tool_name": "education_search_web", "query": query}, success=False)
    ...
    try:
        ...
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "success",
                                    {"tool_name": "education_search_web", "query": query, "result_length": len(result)}, success=True)
        return result
    except Exception as e:
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "error",
                                    {"tool_name": "education_search_web", "query": query, "error": str(e)}, success=False)
        return f"Error during education web search: {e}"


@tool
async def education_query_uploaded_docs(
    query: str,
    user_context: Optional[UserProfile] = None,
    ...
    Args:
        query (str): The search query to find relevant educational documents (e.g., "course syllabus details", "research paper on AI").
        user_context (UserProfile, optional): The user's profile for RBAC checks and logging. Defaults to None.
    ...
    if not get_user_tier_capability(user_context.user_id, 'document_query_enabled', False, user_tier=user_context.tier, user_roles=user_context.roles):
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "permission_denied",
                                    {"tool_name": "education_query_uploaded_docs", "query": query}, success=False)
        return "Error: Document querying is not enabled for your current tier."
    
    if not document_tools:
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "error",
                                    {"tool_name": "education_query_uploaded_docs", "query": query, "error": "DocumentTools instance not provided."}, success=False)
    ...
    try:
        ...
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "success",
                                    {"tool_name": "education_query_uploaded_docs", "query": query, "result_length": len(result)}, success=True)
        return result
    except Exception as e:
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "error",
                                    {"tool_name": "education_query_uploaded_docs", "query": query, "error": str(e)}, success=False)
        return f"Error querying uploaded educational documents: {e}"


@tool
async def education_summarize_document_by_path(
    file_path_str: str,
    user_context: Optional[UserProfile] = None,
    document_tools: Optional[DocumentTools] = None # Accept DocumentTools instance
) -> str:
    ...
    Args:
        file_path_str (str): The full path to the document file to be summarized.
                                Example: "uploads/default/education/lesson_plan.pdf"
        user_context (UserProfile, optional): The user's profile for RBAC checks and logging. Defaults to None.
        document_tools (DocumentTools, optional): The DocumentTools instance. Required for this function.
    ...
    if not get_user_tier_capability(user_context.user_id, 'summarization_enabled', False, user_tier=user_context.tier, user_roles=user_context.roles):
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "permission_denied",
                                    {"tool_name": "education_summarize_document_by_path", "file_path": file_path_str}, success=False)
        return "Error: Document summarization is not enabled for your current tier."

    if not document_tools:
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "error",
                                    {"tool_name": "education_summarize_document_by_path", "file_path": file_path_str, "error": "DocumentTools instance not provided."}, success=False)
    ...
    try:
        ...
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "success",
                                    {"tool_name": "education_summarize_document_by_path", "file_path": file_path_str, "result_length": len(result)}, success=True)
        return result
    except Exception as e:
        analytics_tracker.log_event(user_context.user_id, "tool_usage", "error",
                                    {"tool_name": "education_summarize_document_by_path", "file_path": file_path_str, "error": str(e)}, success=False)
        return f"Error summarizing document: {e}"


# --- EducationTools Class (Wrapper) ---
class EducationTools:
    """
    A collection of tools for education-related operations.
    ...
    async def search_educational_resources(
        ...
        """
        Searches for educational resources.
        """
...
if __name__ == "__main__":
    ...
    class MockSecrets:
        def __init__(self):
            self.coursera_api_key = "MOCK_COURSERA_KEY" # Placeholder
    ...
    class MockConfigManager:
        ...
        def __init__(self):
            ...
            self._api_providers_data = {
                "education": {
                    "coursera": {
                        "base_url": "https://api.coursera.com/api/external/master",
                        "api_key_name": "coursera_api_key",
                        ...
                        "functions": {
                            "search_educational_resources": {
                                ...
                                "response_path": ["elements"], # Assuming 'elements' contains results
                                "data_map": {}
                            }
                        }
                    }
    ...
    with patch.dict(sys.modules, {'firebase_admin.firestore': MagicMock(firestore=MagicMock())}):
        ...
        def mock_requests_dynamic(method, url, params=None, headers=None, json=None, timeout=None):
            ...
            if "api.coursera.com/api/external/master" in url:
                if "/search" in url:
                    ...
                    mock_response.json.return_value = {
                        "status": "success",
                        "elements": [
                            {"title": "Mock Course 1", "description": "Description of mock course 1.", "url": "http://mock.edu/course1"},
                            {"title": "Mock Article 2", "description": "Summary of mock article 2.", "url": "http://mock.edu/article2"}
                        ]
                    }
            ...
            if "serpapi.com/search" in url:
                ...
                mock_response.json.return_value = {
                    "organic_results": [
                        {"title": "Mock Web Search Result 1", "link": "http://example.com/web1", "snippet": f"Snippet for {params.get('q', 'education')} web search result 1."},
                        {"title": "Mock Web Search Result 2", "link": "http://example.com/web2", "snippet": f"Snippet for {params.get('q', 'education')} web search result 2."}
                    ]
                }
                return mock_response
        ...
        async def run_education_tests(education_tools_instance):
            ...
            result_search = await education_tools_instance.search_educational_resources(
                query="data science courses",
                resource_type="course",
                user_context=mock_user_pro_profile
            )
            print(f"Educational Resource Search Result: {result_search}")
            assert "Educational Resources for 'data science courses':" in result_search
            assert "Mock Course 1" in result_search
            ...
            assert logged_data["details"]["tool_name"] == "search_educational_resources"
            assert logged_data["success"] is True
            print("Test 1 Passed.")

            # Test 2: education_search_web (generic tool)
            print("\n--- Test 2: education_search_web (Generic Tool) ---")
            mock_firestore_manager_for_analytics.collection.return_value.add.reset_mock()
            result_web_search = await education_tools_instance.education_search_web("online degrees reviews", user_context=mock_user_pro_profile)
            ...
            logged_data = args[0]
            assert logged_data["event_type"] == "tool_usage"
            assert logged_data["details"]["tool_name"] == "education_search_web"
            assert logged_data["success"] is True
            print("Test 2 Passed.")

            # Test 3: education_query_uploaded_docs (generic tool via DocumentTools)
            print("\n--- Test 3: education_query_uploaded_docs (Generic Tool via DocumentTools) ---")
            mock_firestore_manager_for_analytics.collection.return_value.add.reset_mock()
            result_doc_query = await education_tools_instance.education_query_uploaded_docs("python programming assignments", user_context=mock_user_pro_profile)
            print(f"Document Query Result: {result_doc_query}")
            assert "Mocked document query for education with query 'python programming assignments'" in result_doc_query
            mock_firestore_manager_for_analytics.collection.return_value.add.assert_called_once()
            args, kwargs = mock_firestore_manager_for_analytics.collection.return_value.add.call_args
            logged_data = args[0]
            assert logged_data["event_type"] == "tool_usage"
            assert logged_data["details"]["tool_name"] == "education_query_uploaded_docs"
            assert logged_data["success"] is True
            print("Test 3 Passed.")

            # Test 4: education_summarize_document_by_path (generic tool via DocumentTools)
            print("\n--- Test 4: education_summarize_document_by_path (Generic Tool via DocumentTools) ---")
            mock_firestore_manager_for_analytics.collection.return_value.add.reset_mock()
            test_user_pro_dir = Path("uploads") / mock_user_pro_profile.user_id
            dummy_file_path = test_user_pro_dir / "education" / "dummy_lecture.pdf"
            dummy_file_path.parent.mkdir(parents=True, exist_ok=True)
            dummy_file_path.write_text("This is a dummy lecture content for testing summarization.")
            ...
            logged_data = args[0]
            assert logged_data["event_type"] == "tool_usage"
            assert logged_data["details"]["tool_name"] == "education_summarize_document_by_path"
            assert logged_data["success"] is True
            print("Test 4 Passed.")
