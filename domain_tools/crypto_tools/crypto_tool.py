...
_mock_crypto_data = {
    ...
    "crypto_info": {
        ...
        "ethereum": {
            "name": "Ethereum",
            "symbol": "ETH", # Corrected: Added missing value for 'symbol'
...
@tool
async def get_crypto_price(crypto_id: str, vs_currencies: str = "usd", user_token: str = "default") -> str:
    """
    ...
    api_data = await _make_dynamic_api_request("crypto", "get_crypto_price", params, user_token)
...
@tool
async def get_crypto_info(crypto_id: str, user_token: str = "default") -> str:
    """
    ...
    api_data = await _make_dynamic_api_request("crypto", "get_crypto_info", params, user_token)
...
@tool
async def get_historical_crypto_price(crypto_id: str, date: str, vs_currency: str = "usd", user_token: str = "default") -> str:
    """
    ...
    api_data = await _make_dynamic_api_request("crypto", "get_historical_crypto_price", params, user_token)
...
@tool
async def crypto_query_uploaded_docs(query: str, user_token: str = "default", export: Optional[bool] = False, k: int = 5) -> str:
    """
    ...
    return f"Mocked document query results for '{query}' in section 'crypto'."

@tool
async def crypto_summarize_document_by_path(file_path_str: str) -> str:
    """
    ...
    try:
        # The summarize_document function is async, so await it
...
if __name__ == "__main__":
    ...
    with patch.dict(sys.modules, {'firebase_admin.firestore': MagicMock(firestore=MagicMock())}):
        ...
        # original_QueryUploadedDocs = sys.modules['domain_tools.crypto_tools.crypto_tool'].QueryUploadedDocs # Not needed anymore
        original_summarize_document = sys.modules['domain_tools.crypto_tools.crypto_tool'].summarize_document
        # sys.modules['domain_tools.crypto_tools.crypto_tool'].QueryUploadedDocs = MockQueryUploadedDocs # Not needed anymore
        ...
        # sys.modules['domain_tools.crypto_tools.crypto_tool'].QueryUploadedDocs = original_QueryUploadedDocs # Removed as it's no longer directly imported
